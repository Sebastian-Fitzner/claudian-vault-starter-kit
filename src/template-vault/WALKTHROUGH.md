# Walkthrough — Working with the Vault

This vault is a **Markdown workspace wired for AI-assisted engineering**. Work from any editor or
agent CLI at the vault root. The agent follows the rules in `CLAUDE.md`, picks the right **skill**
for your intent, and reads/writes notes as it works.

Obsidian is optional. If you use it, it adds graph navigation, Dataview tables, and the Claudian
in-vault panel.
...
| "implement the task" | `task-implementation` | Executes the **approved** plan; prepares a worktree if `worktree: true`. Does **not** commit. | `implementing` |
