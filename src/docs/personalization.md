...
- **Turn a feature on later** → re-add its skill folder from a fresh kit unzip into `skills/`, and
  re-add the relevant `CLAUDE.md` section (copy from the kit's `template-vault/CLAUDE.md.tmpl`).
- **Claudian model/effort/panel** → `.claudian/claudian-settings.json`.

## Re-running the scaffolder

`scaffold.mjs` refuses to write into a non-empty target. To regenerate, point `--target` at a fresh
folder and diff against your current vault, or just edit files in place — the kit is yours after the
first run.
